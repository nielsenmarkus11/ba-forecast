...
myxi <- ts(fortif.diff[13:length(fortif.diff)],start=c(1981,1),frequency=12)
mysi <- ts(diff(dat.ts[,1],lag=12)[12:length(diff(dat.ts[,1],lag=12))],start=c(1981,2),frequency = 12)

myma <- rollmean(fortif.diff, k=12, align="right")
ma.pred <- ts(c(fortif.diff,rep(tail(myma,1),2)),start=c(1980,1),frequency=12)
